...
let appData = { subjects: [], tests: {}, tasks: [] };
...
const taskFromTest = (test) => ({
  id: test.task?.id || `${test.id}-task`,
  day: test.task?.day || "Сегодня",
  date: test.task?.date || "",
  subjectId: test.subjectId,
  title: test.task?.title || test.title,
  xp: test.task?.xp || test.xpReward || 10,
  testId: test.id
});

const contentFromTests = (subjects, tests) => {
  const normalizedTests = {};
  const tasks = [];

  tests.forEach((test) => {
    normalizedTests[test.id] = test;
    tasks.push(taskFromTest(test));
  });

  return { subjects, tests: normalizedTests, tasks };
};

const loadFolderContent = async () => {
  const index = await fetchJson(`${contentRoot}index.json`);
  const subjects = await fetchJson(`${contentRoot}${index.subjects || "subjects.json"}`);
  const tests = await Promise.all((index.tests || []).map((testPath) => fetchJson(`${contentRoot}${testPath}`)));
  return contentFromTests(subjects, tests);
};

const loadContent = async () => {
  try {
    const folderContent = await loadFolderContent();
    appData = mergeContent(folderContent, readJson(contentKey));
  } catch (error) {
    console.warn("Studify content folder was not loaded. Use a local server for /content JSON files.", error);
    appData = mergeContent(window.STUDIFY_CONTENT || { subjects: [], tests: {}, tasks: [] }, readJson(contentKey));
  }
};
...
contentImport?.addEventListener("change", async (event) => {
  ...
  try {
    ...
    appData = mergeContent(appData, nextOverrides);
    renderAllData();
